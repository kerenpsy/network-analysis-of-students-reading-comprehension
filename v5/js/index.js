...
let prescore1 = 0 //前测1得分
let prescore2 = 0 //前测2得分
let precorr1 = 0
let precorr2 = 0
let shortQuest = []
let E5J = {} //简答题
...
let welcome = {
  ...
  message: `
  <h1 style="margin-bottom:10px">你好！👋</h1>
...
var participantsInfo = {
  ...
  html:
    ...
    <div style="display: flex; align-items: center; margin-bottom: 25px;">
      <div style="width: 80px; text-align: right;">就读院校:</div>
      ...
      <div style="display: flex; align-items: center; margin-bottom: 25px;">
      <div style="width: 80px; text-align: right;">就读年级:</div>
      <input name="name" type="text" placeholder="请填写年级" style="flex: 1; font-size:24px; margin-left: 10px; padding: 5px; border: 1px solid #ccc;" required>
    </div>
    ...
    <select id="age" name="age" style="flex: 1; font-size:24px; margin-left: 10px; padding: 5px; border: 1px solid #ccc;">
      ...
      <option value="9">18岁-22岁</option>
      <option value="10">22岁-25岁</option>
      <option value="11">25岁-30岁</option>
      <option value="12">30岁以上</option>
    </select>
  </div>
...
let instruction = {
  ...
  stimulus: `
    ...
    你想了解和提高自己的阅读水平吗？
      <br>
      本测试有<span class="redT">2篇文章📜</span>，帮助你了解自己的阅读能力。
      <br>
      📝每篇文章后有<span class="redT">三类</span>题目📝：
      <br>
      第一类是<span class="redT">选择题</span>，每道选择题只有一个正确答案；
      <br>
      第二类是<span class="redT">评分题</span>，需根据文意，对配对词组之间的<span class="redT" style="font-style: italic;">关系</span>评分；
      <br>
      第三类是<span class="redT">简答题</span>，尽量用上所提供的关键词，写下你对文章内容的理解；
      <br>
      结束后，你会得到一份阅读理解能力报告，让你更了解自己的阅读水平。
    </p>
...
let part1E = {
  ...
  请你仔细阅读以下文章，<br>
  分别完成<span class="redT">选择题</span>、<span class="redT">评分题</span>的题目。
  </p>
`,
  choices: ['开始']
};
...
function pretestMultiChoice(test) {
  return {
    ...
    <div style="width:80%;margin:0 auto;font-size:24px;line-height:45px;font-weight:bold;text-align:left;font-family: 'FangSong'">一、选择题：每个字只有一个正确答案。</div>
    <div id="article">
      <div style="width:80%;margin:0 auto;font-size:24px;line-height:45px">
        <h1 class="aTitle">${test.tTitle}</h1>
      </div>
...
//测验 - 评分题1
function createMultiLikert1(article, type) {
  return {
    type: jsPsychSurveyLikert,
    preamble: function () {
      let curQuest = ''
      if (type === 'E') {
        curQuest = '一、评分题：以下为关联性评分，即根据词语之间的逻辑关系远近进行评分。<br></div><div style="width:80%;margin:0 auto;font-size:24px;line-height:45px;;text-align:left;font-style: italic;">1分表示关键词之间没有关系；9分表示关键词之间关系密切；5分表示关键词之间有关系，但关系相对较小。'
      } else {
...
//测验 - 评分题2
function createMultiLikert2(article, type) {
  return {
    type: jsPsychSurveyLikert,
    preamble: function () {
      let curQuest = ''
      if (type === 'E') {
        ...
        curQuest = '一、评分题：以下为关联性性评分，即根据词语之间的逻辑关系远近进行评分。<br></div><div style="width:80%;margin:0 auto;font-size:24px;line-height:45px;;text-align:left;font-style: italic;">1分表示关键词之间没有关系；9分表示关键词之间关系密切；5分表示关键词之间有关系，但关系相对较小。'
      }
      let text =
        `
    <div style="width:80%;margin:0 auto;font-size:24px;line-height:45px">
...
for (let article of articlesListE) {
  if (article.aId === "E6") {
    ...
    timeline.push(createMultiLikert1(article, 'E'))
    timeline.push(createMultiLikert2(article, 'E'))
    timeline.push(textQuestFun(E6E1Text))
    // shortQuest.push(createMultiChoice(article),createMultiLikert1(article,'E'), textQuestFun(E6C1Text))
  }
  // else {
  //   timeline.push(createMultiChoice(article))
  //   timeline.push(createMultiLikert1(article, 'E'))
...
for (let article of articlesListN) {
  if (article.aId === "N6") {
    timeline.push(createMultiChoice(article))
    timeline.push(createMultiLikert1(article, 'N'))
    timeline.push(createMultiLikert2(article, 'N'))
    timeline.push(textQuestFun(E6C1Text))
    // shortQuest.push(createMultiChoice(article),createMultiLikert1(article,'N'), textQuestFun(E6E1Text))
  }
  // else {
  //   timeline.push(createMultiChoice(article))
  //   timeline.push(createMultiLikert1(article, 'N'))
